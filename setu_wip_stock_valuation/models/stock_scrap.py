...
class StockScrap(models.Model):
    ...
    def action_validate(self):
        if self.scrap_qty and self.production_id.state != 'done':
            ...
            self.env["stock.picking"].do_unreserve_and_reserve_from_lot(lot_ids,
                                                                        self.production_id.move_raw_ids,
                                                                        qty=production_reserved_qty - self.scrap_qty)
            self.production_id.create_raw_material_movement(self.product_id, self.scrap_qty, self.location_id,
                                                            self.scrap_location_id, is_return=False)
