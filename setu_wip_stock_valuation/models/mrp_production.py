...
class MrpProduction(models.Model):
    ...
    def create_internal_picking_and_validate(self, product_id, location_id, dest_location_id, group_id, qty, lot_id,
        ...
        picking.do_unreserve_and_reserve_from_lot(lot_id)
    ...
    def create_raw_material_movement(self, product_id, qty, location_id, location_dest_id, is_return=False):
        self.env["setu.raw.material.movement"].create({'production_id': self.id,
                                                       'product_id': product_id.id,
                                                       'quantity': qty * (1 if not is_return else -1),
                                                       'location_id': location_id.id,
                                                       'location_dest_id': location_dest_id.id
                                                       })
    ...
    def get_available_component_qty_for_return(self):
        ...
        records = self.env["setu.raw.material.movement"].search(
            [('production_id', 'in', production_ids.ids)])
        return sum(records.mapped('quantity')) - sum(
            production_ids.mapped('move_raw_ids').filtered(lambda move: move.state == 'done').mapped('quantity_done'))

    def action_add_product(self):
        return self.workorder_ids[0].action_add_component()